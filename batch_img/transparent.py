...
class Transparent:
    ...
    def do_1_image_transparency(args: tuple) -> tuple:
        ...
        Returns:
            tuple: bool, str
        """
        in_path, out_path, transparency = args
        ...
        try:
            with Image.open(in_path) as img:
                ...
                file = Common.set_out_file(in_path, out_path, f"a{transparency}")
                i_format = img.format
                if i_format == "JPEG":
                    i_format = "PNG"
    ...
    def all_images_transparency(
        in_path: Path, out_path: Path | str, transparency: int
    ) -> bool:
        ...
        tasks = [(f, out_path, transparency) for f in image_files]
        files_cnt = len(tasks)
        if files_cnt == 0:
