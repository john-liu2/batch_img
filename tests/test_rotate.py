...
@pytest.fixture(
    params=[
        (
            ...
            Path(f"{_dir}/.out/"),
            90,
            (True, Path(f"{_dir}/.out/IMG_0070_90cw.HEIC")),
        ),
        (
            Path(f"{_dir}/data/PNG/Checkmark.PNG"),
            Path(f"{_dir}/.out/"),
            180,
            (True, Path(f"{_dir}/.out/Checkmark_180cw.PNG")),
        ),
        (
            Path(f"{_dir}/data/JPG/152.JPG"),
            Path(f"{_dir}/.out/"),
            270,
            (True, Path(f"{_dir}/.out/152_270cw.JPG")),
        ),
        (
            Path(f"{_dir}/data/HEIC/chef_show2.heic"),
            Path(f"{_dir}/.out/"),
            180,
            (True, Path(f"{_dir}/.out/chef_show2_180cw.heic")),
        ),
        (
            Path(f"{_dir}/data/JPG/152.JPG"),
            Path(f"{_dir}/.out/"),
